...
export default async function handler(req, res) {
  ...
  setBaseURL('sqls/comment/delete'); // 끝에 슬래시 붙이지 마시오.
...
async function main(req, res) {
  ...
  const qIsToCom = await QTS.getIsToCom.fQuery({ commentId });
  ...
  if (!isToCom) {
    ...
    const qHasToCom = await QTS.getHasToCom.fQuery({ commentId });
    ...
    const qComm = await QTS.getFromCom.fQuery({ commentId });
    if (qComm.type === 'error')
    ...
    if (qComm.message.rows.length > 0) {
      ...
      // #3.5. 댓글이 대댓글을 가지고 있는지 판단한다.
      const qHasToCom = await QTS.getHasToCom.fQuery({ commentId: comId });
      if (qHasToCom.type === 'error')
        return qHasToCom.onError(res, '3.5', 'searching if it has tocomment');
      const hasToCom = qHasToCom.message.rows.length > 0;
      if (!hasToCom) {
        // #3.12. 모든 대댓글 영구삭제
        const qDelComments = await QTS.delToComs.fQuery({ commentId: comId });
        if (qDelComments.type === 'error')
          return qDelComments.onError(res, '3.12', 'delete comments');
        // #3.13. 유효한 대댓글이 없으면 영구삭제;
        const qDel = await QTS.delComment.fQuery({ commentId: comId });
        if (qDel.type === 'error')
          return qDel.onError(res, '3.13', 'delete comment');
        // #3.14. tocomment도 영구삭제
        /* const qDelToCom = await QTS.delToComment.fQuery({ commentId: comId });
        if (qDelToCom.type === 'error')

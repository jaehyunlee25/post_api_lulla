...
const baseUrl = 'sqls/post/bookmark'; // 끝에 슬래시 붙이지 마시오.
...
async function main(req, res) {
  ...
  const { member_id: memberId, bookmark } = req.body;
  const { post_id: postId } = bookmark;
  ...
  if (qPost.type === 'error')
    return qPost.onError(res, '3.4', 'searching post');
  ...
  // #3.6. bookmark 검색
  const qBM = await QTS.getBM.fQuery(baseUrl, { memberId, postId });
  if (qBM.type === 'error')
    return qBM.onError(res, '3.4', 'searching bookmark');

  if (qBM.message.rows.length === 0) {
    // #3.7. bookmark 생성
    const qNew = await QTS.newBM.fQuery(baseUrl, { memberId, postId });
    if (qNew.type === 'error')
